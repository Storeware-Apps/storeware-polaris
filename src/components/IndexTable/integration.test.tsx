...
import { IndexFilters, useSetIndexFiltersMode } from "../IndexFilters/indexFilters";
...
const CompleteIntegrationExample = () => {
  ...
  const rowMarkup = currentOrders.map((order, index) => (
    ...
    >
      ...
      <IndexTable.Cell>
        <Badge progress={order.paymentStatus === 'Paid' ? 'complete' : 'partiallyComplete'}>
          {order.paymentStatus}
        </Badge>
      </IndexTable.Cell>
      <IndexTable.Cell>
...
describe("IndexTable Integration", () => {
  ...
  it("handles filtering mode toggle", () => {
    ...
    expect(screen.getByPlaceholderText("Searching in all orders")).toBeInTheDocument();
  });
    ...
    const TestComponent = () => {
      ...
      return (
        <div>
          ...
          <div data-testid="all-selected">{allResourcesSelected.toString()}</div>
    ...
    const TestComponent = () => {
      ...
      return (
        <div>
          ...
          <button onClick={() => setMode(mode === "DEFAULT" ? "FILTERING" : "DEFAULT")}>
            Toggle Mode
          </button>
        </div>
      );
